use solana_program::{
    ...
    pubkey::Pubkey,
    system_instruction,
    sysvar::rent::Rent, program_error::ProgramError,
};
...
impl Processor {
    ...
    fn process_initialize_user(
        ...
        let unpacked_platform_state_account = PlatformState::unpack(&platform_state_account.try_borrow_data()?)?;

        if unpacked_platform_state_account.platform_treasury_sol_wallet != *platform_sol_treasury_wallet_account.key {
        ...
        let mut unpacked_user_state_account = UserState::unpack(&user_state_account.try_borrow_data()?)?;
